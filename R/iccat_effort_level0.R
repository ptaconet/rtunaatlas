#' @name iccat_effort_level0
#' @aliases iccat_effort_level0
#' @title Extract source georeferenced effort datasets of ICCAT from Sardara World database
#' @description This function extracts the source georeferenced effort datasets stored in the Tuna atlas database coming from the International Commission for the Conservation of Atlantic Tuna (ICCAT). The output dataset provides the effort targetting tuna, tuna-like and by-catch species in the area managed under the jurisdiction of the ICCAT. Efforts are stratified by month, fishing gear, vessel flag reporting country, fishing mode (i.e. type of school used), area (usualy 1° or 5° square) and unit of effort.
#' @export
#'
#' @usage iccat_effort_level0(year_tunaatlas,include_type_of_school)
#'                 
...
#' @details 
#' The output dataset lists effort targetting tuna, tuna-like and shark species in the Atlantic ocean. efforts are stratified by month, fishing gear, vessel flag reporting country, fishing mode (i.e. type of school used) - if parameter \code{include_type_of_school} is set to TRUE, area (1° or 5° square) and unit of effort. This dataset is computed using public domain datasets released by the International Commission for the Conservation of Atlantic Tuna (ICCAT).
#' 
#' Details on the use of the parameter \code{include_type_of_school}: ICCAT delivers two effort-and-efforts datasets for purse seiners: one that gives the detail of the type of school (Fad|Free school) for purse seine fisheries and that starts in 1994 (called Task II effort|effort by operation mode Fad|Free school) and one that does not give the information of the type of school and that covers all the time period (from 1950) (called Task II effort|effort). These data are redundant (i.e. the data from the dataset Task II effort|effort by operation mode are also available in the dataset Task II effort|effort) but in the latter, the information on the type of school is not available.
#' \itemize{
...
#' @examples
#' 
#' # Retrieve ICCAT georeferenced effort data from 2017, with information on fishing mode
#' iccat_effort<-iccat_effort_level0(2017,include_type_of_school=TRUE)
#' head(iccat_effort)
#' 
...
iccat_effort_level0<-function(year_tunaatlas,include_type_of_school){
  
  # Select iccat raw datasets release on the year year_tunaatlas
  con <- db_connection_tunaatlas_world()
  
  datasets_permanent_identifiers="'atlantic_ocean_effort_tunaatlasICCAT_level0__noSchool'"
  
  metadata_datasets_WithoutSchooltypeInfo<-dbGetQuery(con,paste0("SELECT * from metadata.metadata where persistent_identifier IN (",datasets_permanent_identifiers,") and identifier LIKE '%_",year_tunaatlas,"_level0%'"))
  ...
  if (include_type_of_school==TRUE){
    ...
    datasets_permanent_identifiers="'atlantic_ocean_effort_1deg_1m_ps_tunaatlasICCAT_level0__bySchool'"
