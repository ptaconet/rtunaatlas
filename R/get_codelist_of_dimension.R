...
#'
#' dataset_metadata<-list_metadata_datasets(con,dataset_name="indian_ocean_catch_ll_1952_11_01_2016_01_01_tunaatlasIOTC_2017_level0")
#'
...
get_codelist_of_dimension<-function(con,dataset_metadata,dimension_name){
  ...
  dataset_name_codelist_dimension_query<-paste0("select distinct(",db_dimensions_parameters$db_tablesource_colname,") from ",dataset_metadata$table_name," tab join ",db_dimensions_parameters$db_tablename," tab_link on tab_link.",db_dimensions_parameters$db_pkattribute_colname,"=tab.",db_dimensions_parameters$db_pkattribute_colname," where tab.",db_dimensions_parameters$db_pkattribute_colname,"<>0 and tab.id_metadata=",dataset_metadata$id_metadata)
  ...
  metadata_codelist_dimension=dbGetQuery(con,paste0("SELECT * FROM metadata.metadata where dataset_name='",dataset_name_codelist_dimension,"'"))
